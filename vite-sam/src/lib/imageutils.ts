export function maskImageCanvas(imageCanvas: HTMLCanvasElement, maskCanvas: HTMLCanvasElement): HTMLCanvasElement {
  ...
  export function resizeCanvas(canvasOrig: HTMLCanvasElement, size: { w: number, h: number }): HTMLCanvasElement {
    const canvas = document.createElement("canvas");
  ...
  export function resizeAndPadBox(sourceDim: { w: number, h: number }, targetDim: { w: number, h: number }): { x: number, y: number, w: number, h: number } {
